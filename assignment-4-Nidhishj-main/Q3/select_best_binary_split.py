    ...
    for f in range(F):
        ...
        for v,thresh in enumerate(possib_xthresh_V):   
            ...
            if np.sum(left_mask) < MIN_SAMPLES_LEAF or np.sum(right_mask) < MIN_SAMPLES_LEAF:
                ...
                continue
            left_cost_V = np.sum(np.square(y_N[left_mask] - np.mean(y_N[left_mask])))
            right_cost_V = np.sum(np.square(y_N[right_mask] - np.mean(y_N[right_mask])))
            total_cost_V[v] = left_cost_V + right_cost_V
