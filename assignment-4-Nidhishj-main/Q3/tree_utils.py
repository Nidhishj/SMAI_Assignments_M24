...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        ''' 
        T, F = x_TF.shape
        if self.feat_id < 0 or self.feat_id >= F:
            raise ValueError("feat_id %d is invalid for input with %d features"
                             % (self.feat_id, F))
        ...
        #### fixes now from my side first I will make a binary mask of what will be left and right and then finally merge 
        left_mask_T = np.zeros(T, dtype=bool)
        right_mask_T = np.zeros(T, dtype=bool)
        for t in range(T):
            if x_TF[t, self.feat_id] < self.thresh_val:
                left_mask_T[t] = True
            else:
                right_mask_T[t] = True
        preds = []
        preds.append(self.left_child.predict(x_TF[left_mask_T]))
        preds.append(self.right_child.predict(x_TF[right_mask_T]))
        yhat_T = np.zeros(T, dtype=np.float64)
        yhat_T[left_mask_T] = preds[0]
        yhat_T[right_mask_T] = preds[1]
        ...
        # yhat_T = 1.2345 * np.ones(T, dtype=np.float64) # TODO fixme ye kya tha ??
        return yhat_T
...
class LeafNode(object):
    ...
    '''
    ### n is for training and y_n is just the training examples reached at the node
    ...
    def predict(self, x_TF):
        ...
        '''
        T = x_TF.shape[0]
        yhat_T = np.ones(T) * np.mean(self.y_N)
